    ...
    def create_train_and_val_set(self):
        ...
        for file in os.listdir(self.source_path):
            if file.endswith('.' + self.classification_file_type):
                self.classification_list.append(os.path.join(self.source_path, file))
            elif file.endswith('.' + self.image_file_type):
                self.image_list.append(os.path.join(self.source_path, file))
...
if __name__ == '__main__':
    temp = Create_Training_Datasets('C:\Bachelor Oppgave\Datasets\Temp1', 'C:\Bachelor Oppgave\SpaceArcheology\Datasets\Mound PNG\Slope', 'csv', 'png')
